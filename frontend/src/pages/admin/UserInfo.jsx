...
const UserInfo = () => {
  ...
  const getUserDetails = async () => {
    try {
      const res = await axios.get(
        `https://eco-mart-r73h.onrender.com/api/v1/user/get-user/${userId}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (file) {
        formData.append("file", file); //  backend multer key
      }

      const res = await axios.put(
        `https://eco-mart-r73h.onrender.com/api/v1/user/update/${userId}`,
        formData,
        {
          headers: {
